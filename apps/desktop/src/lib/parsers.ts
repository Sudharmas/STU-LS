...
export function parseDepartmentCounts(input: string): DepartmentCount[] {
  return input
    .split("\n")
    .map((line) => line.trim())
    .filter(Boolean)
...
export function parseUsnsFromText(input: string): string[] {
...
export function extractUsnsFromWorkbookBuffer(buffer: ArrayBuffer): string[] {
